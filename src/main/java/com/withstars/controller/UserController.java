...
public class UserController {
    ...
    @ResponseBody
    public Object UpLoadImg(@RequestParam(value="myFileName")MultipartFile mf,HttpServletRequest request, HttpSession session) {
    	String tempNum=(int)(Math.random()*100000)+"";
    	String fileName=mf.getOriginalFilename();
        String suffix=fileName.substring(fileName.lastIndexOf(".")+1, fileName.length());
        Long date=new Date().getTime();
        String newFileName=date+"-"+tempNum+"."+suffix;
        String absolutePath=session.getServletContext().getRealPath("/static/img/avatar")+"/"+newFileName;
        String relativePath="/img/avatar"+"/"+newFileName;
        File file=new File(absolutePath);
        if (!file.exists()){
    ...
    map.put("data",relativePath);//这里应该是项目路径
    return map;//将图片地址返回
    }
    ...
    public String editUser(HttpServletRequest request){
    	...
    	userService.editUser(email, null, id);
    	return "1";
    }
    ...
    public String collect(HttpServletRequest request){
    	...
    	int count = userService.getCount(replyUserId, topicid);
    	if(count>0){
    		return "-1";
    	}
    	
    	userService.addCollect(replyUserId, topicid);
    	
    	return "1";
    }
    
    @RequestMapping(value ="/user/delbbs",method=RequestMethod.POST)
    @ResponseBody
    public String delbbs(HttpServletRequest request){
    	String topicid = request.getParameter("topicid");
    	String replyUserId = request.getParameter("replyUserId");
    	userService.delbbs(replyUserId, topicid);
    	return "1";
    }
    
    @RequestMapping(value ="/user/quxiaobbs",method=RequestMethod.POST)
    @ResponseBody
    public String quxiaobbs(HttpServletRequest request){
    	String topicid = request.getParameter("topicid");
    	String replyUserId = request.getParameter("replyUserId");
    	userService.quxiaobbs(replyUserId, topicid);
    	return "1";
    }
    
    @RequestMapping(value ="/user/delReply",method=RequestMethod.POST)
    @ResponseBody
    public String delReply(HttpServletRequest request){
    	String topicid = request.getParameter("id");
    	replyService.deleteByPrimaryKey(Long.valueOf(topicid));
    	return "1";
    }
