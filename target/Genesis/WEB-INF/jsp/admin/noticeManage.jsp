...
<section>
    ...
    <div class="left-side sticky-left-side">
        ...
        <div class="left-side-inner">
            ...
            <ul class="nav nav-pills nav-stacked custom-nav">
                ...
                <li class="active"><a href="/admin/noticeManage"><i class="fa fa-home"></i> <span>公告管理</span></a></li>
    ...
    <div class="main-content" >
        ...
        <tbody>
       <c:forEach items="${topics }" var="u" varStatus="s">
        <tr class="gradeX">
          <td>${s.index+1}</td>
          <td >${u.title }</td>
          <td>${u.content }</td>
          <td>
          	<a onclick="del(${u.id })">删除</a>
          </td>
        </tr>
      ...
      <tr>
		<td colspan="8" class="center" > <c:if test="${page.page>1}">
              <a href="/admin/noticeManage?page=1" >首页</a>
             <a href="/admin/noticeManage?page=${page.page-1 }"> 上一页</a> 
             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="/admin/noticeManage?page=${page.page+1 }">下一页</a>
			<a href="/admin/noticeManage?page=${page.totalPage }">末页</a>
		    </c:if> 
		</td>
      </tr>
        </tbody>
...
$(function(){
	...
	$.ajax({
	    ...
	    url:"/admin/getAllTabs",//获取option值得方法返回LIST<String>类型 
	    success:function(data){
	        for(var i in data){        //遍历输出LIST添加为option     
	            $("#utype").append("<option value='"+data[i].id+"'>"+data[i].tabName+"</option>");
	        }
	    }
	});
...
function del(id){
	...
	function subMit(){
		...
		var utype = $("#utype").find("option:selected").val();
		 ...
		 var age1 = nullOrEmpt(title);
		 var number1 = nullOrEmpt(content);
		...
		$.ajax({
		       type: "POST",
		       url:"/admin/addNotice",
		       data:{'title':title,'content':content,utype:utype},
		       dataType:"json",
		       success: function(data) {
		       	 if(data=='1'){
